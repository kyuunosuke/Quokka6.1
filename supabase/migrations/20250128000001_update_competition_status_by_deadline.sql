...
DROP TRIGGER IF EXISTS trigger_check_competition_deadline ON public.competitions;
DROP FUNCTION IF EXISTS check_competition_deadline() CASCADE;
...
BEGIN
    ...
    WHERE end_date <= timezone('utc'::text, now())
    AND status = 'active';
END;
$$ LANGUAGE plpgsql;
...
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.end_date <= timezone('utc'::text, now()) AND NEW.status = 'active' THEN
